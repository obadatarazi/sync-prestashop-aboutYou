...
class DbSyncLogger
{
    ...
    public function __construct(string $runId, string $channel = 'sync', bool $skipDatabase = false)
    {
    ...
    private function write(string $level, string $msg, array $ctx): void
    {
        // DB log
        try {
            if (!$this->skipDatabase && in_array($level, ['warning', 'error', 'critical'], true)) {
                $this->flushPending();
                $this->repo->log($this->runId, $level, $this->channel, $msg, $ctx);
            } elseif (!$this->skipDatabase) {
                $this->pendingRows[] = [
                    'run_id' => $this->runId,
                    'level' => $level,
    ...
    {
        if ($this->pendingRows === [] || $this->skipDatabase) {
