...
class ProductSyncService
{
    ...
    public function __construct(
        ...
        $this->logger          = new DbSyncLogger($runId, 'sync', $dryRun);
    ...
    public function syncForProductIds(array $psIds): array
    {
        ...
        foreach (array_unique($psIds) as $id) {
            ...
            if ($p) {
                ...
                $this->stats['failed']++;
                $this->recordReason((int) $id, 'failed', 'ps_not_found', 'PrestaShop product not found for requested id');
            }
        }
            ...
            try {
                ...
                if ($this->dryRun) {
                    try {
                        ...
                        $combinations = $this->applyVariantEanOverrides(0, $combinations);
                        ...
                        $externalEanConflicts = $this->products->findExternalEanConflicts(0, $variantEans);
                        ...
                        $this->logger->info("DRY_RUN preflight OK PS#{$psId}", [
                            'variant_count' => count($ayProduct['variants'] ?? []),
                        ]);
                        $this->stats['skipped']++;
                        $this->recordReason($psId, 'skipped', 'dry_run_preflight_ok', 'DRY_RUN preflight passed');
                    } catch (ValidationException $e) {
                        $this->stats['failed']++;
                        $this->logger->error("DRY_RUN PS#{$psId} validation failed", ['errors' => $e->errors()]);
                        $this->recordReason($psId, 'failed', 'local_preflight', implode('; ', array_map(static fn ($v): string => (string) $v, $e->errors())));
                    } catch (\Throwable $e) {
                        $this->stats['failed']++;
                        $this->logger->error("DRY_RUN PS#{$psId}: " . $e->getMessage());
                        $this->recordReason($psId, 'failed', $this->classifyTransportError($e), $e->getMessage());
                    }
                    continue;
                }
                ...
                if ($hasError) {
                    ...
                    if ($this->shouldBypassAyValidationErrors($results)) {
                        $this->logger->warning('AY validation errors bypassed in test mode', [
                            'ps_id' => $psId,
                            'errors' => $pushErrors,
                        ]);
                        ...
                        $this->stats['pushed']++;
                        $this->recordReason($psId, 'pushed', 'test_bypass_validation', 'AY validation errors bypassed in test mode', [
                            'errors' => $pushErrors,
                        ]);
                        continue;
                    }
                    ...
                    $this->stats['failed']++;
                    $this->recordReason($psId, 'failed', $this->ayPolicy->classifyAyError($errorMessage), $errorMessage);
                } else {
                    ...
                    $this->recordReason($psId, 'pushed', 'success', 'Product synced to AboutYou', [
                        'style_key' => $ayProduct['style_key'] ?? null,
                        'verification' => $verification,
                    ]);
                }
                ...
                $this->recordReason($psId, 'failed', 'local_preflight', $error, ['errors' => $e->errors()]);
                try {
                ...
                $this->recordReason($psId, 'failed', $reasonCode, $e->getMessage(), ['exception' => get_class($e)]);
                if ($retryable) {
                    $this->retryJobs->enqueue('product_push', (string) $psId, [
                        'ps_product_id' => $psId,
                        'reason_code' => $reasonCode,
    ...
    {
        $this->stats = ['fetched' => 0, 'pushed' => 0, 'skipped' => 0, 'failed' => 0, 'reasons' => []];
    }
    ...
    private function recordReason(int $psId, string $status, string $reasonCode, string $message, array $extra = []): void
    {
        $entry = [
            'ps_id' => $psId,
            'status' => $status,
            'reason_code' => trim($reasonCode) !== '' ? $reasonCode : 'unknown',
            'message' => trim($message) !== '' ? $message : 'No details',
        ];
        if ($extra !== []) {
            $entry['details'] = $extra;
        }
        $this->stats['reasons'][] = $entry;
    }

    private function shouldBypassAyValidationErrors(array $results): bool
    {
        if (!filter_var($_ENV['AY_TEST_BYPASS_VALIDATION_ERRORS'] ?? false, FILTER_VALIDATE_BOOLEAN)) {
            return false;
        }
        if ($results === []) {
            return false;
        ...
        foreach ($results as $res) {
            ...
            $reason = (string) ($res['reason_code'] ?? $this->ayPolicy->classifyAyError((string) $res['error']));
            if ($reason !== 'ay_validation') {
                return false;
            }
        }
        return true;
