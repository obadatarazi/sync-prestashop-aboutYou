...
use App\Support\AyPayloadValidator;
use App\Support\MaterialCompositionParser;
use App\Support\MaterialCompositionResolver;
...
final class AboutYouMapper
{
    ...
    private function mapVariant(
        ...
        $forcedSizeId = (int) ($_ENV['AY_TEST_FORCE_SIZE_ID'] ?? 0);
        if ($forcedSizeId > 0) {
            // Temporary test override: force all variants to a known-valid AY size id.
            $sizeId = $forcedSizeId;
            $secondSizeId = 0;
            $warnings[] = '[reason=test_override] Forced size mapping via AY_TEST_FORCE_SIZE_ID=' . $forcedSizeId;
        }
